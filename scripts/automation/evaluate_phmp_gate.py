#!/usr/bin/env python3
"""Evaluate iot-phmp-gate.json. Exit 0 PASS, 1 FAIL, 2 missing/SKIP/invalid."""
...
def self_test(root: Path) -> int:
    fixture = root / "scripts" / "automation" / "fixtures" / "iot-phmp-gate-pass.json"
    code = evaluate(fixture)
    if code != 0:
        print("self-test expected PASS fixture to exit 0", file=sys.stderr)
        return 1
    fail = root / "scripts" / "automation" / "fixtures" / "iot-phmp-gate-fail.json"
    code = evaluate(fail)
    if code != 1:
